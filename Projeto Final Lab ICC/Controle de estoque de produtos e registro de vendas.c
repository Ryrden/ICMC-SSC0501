...
//Variaveis globais para contagem após finalizando do programa;
int cont_prod = -1;
...
void menu() { //Menu com opções
    ...
    printf("9: Gerar Relatório de vendas de um determinado m�s\n");
    ...
    for (i = 0; i < tam; i++) {
        for (int k = i + 1; k <= tam; k++) {
            ...
            if (v[i].quantidade < v[k].quantidade) {
                ...
                strcpy(aux1, p[i].nome);
                strcpy(p[i].nome, p[k].nome);
                strcpy(p[k].nome, aux1);
...
void RelatorioVendasM(struct produtos *p, struct vendas *v, int tam, char mes[11]) { //Gera um Relat�rio txt de vendas de um determinado m�s do ano
    ...
    if (strcmp(res, "s") == 0) {
        ...
        for (i = 0; i <= tam; i++) {
            for (int j = 0; j < 10; j++) {
                if (mes[0] == v[i].data[j][3] && mes[1] == v[i].data[j][4]) { //Loop p/ descobrir a venda e lucro total
                    k = 9;
                    while (v[i].data[k][3] != '0' && v[i].data[k][3] != '1') {
                        k--;
                        if (k == 0) {
